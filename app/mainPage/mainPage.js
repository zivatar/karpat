...
.controller('mainPageCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$scope.gravatarize = function(email) {
		email = MD5(email.toLowerCase());
		return 'http://www.gravatar.com/avatar/' + email + '?s=40&d=mm'; //&d=404'
	}
	
	$scope.getRecentArticles = function() {
		$http({
  			method: 'GET',
  			url: Constant.API_URL + '?action=' + Constant.API_ENDPOINT.RECENT_ARTICLES
		}).then(function successCallback(response) {
    		$scope.recentArticles = response.data;
